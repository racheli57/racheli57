...
def build_research_expense_article(url: str, variant: int) -> dict[str, object]:
    ...
    article_title = titles[(variant - 1) % len(titles)]
    common_facts = "政策重点很明确：2025年度研发费用较2024年度增量达到200万元含以上，或符合新设企业、首次纳入R&D经费统计等特殊口径的单位，可以按研发费用增长幅度分档获得事后资助；每家单位年度资助最低不少于20万元，最高不超过300万元。"
    company = "深圳金赋成立于2017年，是扎根深圳的国家高新技术企业，长期运营补贴平台，沉淀1100万条全国四级公开政策数据。我们更擅长把政策原文拆成企业能看懂的匹配条件、材料清单和申报提醒，帮助企业少走弯路。"
    cta = "想先判断企业是否适合申报，可关注「金赋补贴宝」，进入补贴平台做资质评估和政策匹配，把研发费用、项目资料和可申报补贴机会先梳理清楚。"
    variants = [
        [
    ...
    while sum(len(x) for x in paragraphs) < TARGET_ARTICLE_CHARS and idx < len(expansions):
        extra=expansions[(variant+idx-1)%len(expansions)]
        if extra not in paragraphs:
            paragraphs.insert(-1, extra)
        idx += 1
    return {
