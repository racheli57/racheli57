...
PROMOTION_PLANS = [
    ...
    ('工信专项资金不是看标题，关键在费用边界和项目周期', '制造业、软件信息、工业互联网、绿色制造和产业链配套企业', '专项资金方向、支持对象、费用范围、资助比例、项目周期和验收要求', '项目申请书、合同发票、付款凭证、设备清单、研发材料、审计资料和绩效报告'),
    ...
    ('前海企业做跨境和现代服务业，政策匹配要更主动', '前海现代服务业、跨境服务、金融科技、专业服务和创新平台企业', '区域政策、跨境便利、现代服务业、人才支持、办公空间和产业贡献', '注册证明、租赁合同、服务合同、经营数据、人员材料、纳税证明和项目说明'),
    ('工程管理资料别只归档，也要服务未来政策申报', '施工、设计、监理、造价咨询、检测和工程管理企业', '工程质量、安全管理、资质等级、项目备案、绿色施工和信用评价', '工程合同、施工资料、验收文件、人员证书、费用凭证、信用记录和项目总结'),
    ('福田企业看政策，先确认总部、金融和专业服务定位', '福田区金融、科技、商贸、专业服务和总部经济企业', '总部经济、金融服务、科技创新、商贸消费、楼宇经济和人才支持', '注册地址、租赁合同、营收纳税、人员社保、项目合同、资质证书和发展计划'),
    ('大鹏企业做生态文旅，合规和特色项目都要提前规划', '大鹏新区文旅、海洋、生态农业、民宿、体育和科技服务企业', '生态保护、文旅消费、特色产业、项目投入、品牌推广和经营成效', '项目方案、经营数据、合同票据、宣传资料、安全记录、生态合规和效果报告'),
...
def build_article_from_url(url: str, index: int) -> dict[str, object]:
    ...
    if summary:
        opening_options = [
            ...
            f"看到{policy_name}后，企业要做的不是简单收藏通知，而是马上判断它和自身业务的关系。原文释放的关键信息包括：{summary}。如果这些内容与企业近期投入、项目成果或资质建设有关，就应尽快纳入补贴申报计划。",
        ]
    else:
        opening_options = [
            f"{policy_name}已经发布，政策来源为{source}。围绕“{article_title}”，企业先别急着问能拿多少钱，而要核验政策对象、申报条件、支持方式、材料清单和办理节奏。把这些信息转成内部任务，才是拿补贴的第一步。",
            f"{source}发布的{policy_name}，对{audience}来说值得重点关注。企业应按政策原文要求先排查对象、条件、材料、流程和监管要求。只要这些要素能对应到企业现有项目，就有必要进入补贴评估。",
            f"看到{policy_name}后，企业要做的不是简单收藏通知，而是马上围绕“{article_title}”判断它和自身业务的关系。建议先检查适用区域、申报主体、项目周期、费用范围和证明材料。如果这些内容与企业近期投入、项目成果或资质建设有关，就应尽快纳入补贴申报计划。",
        ]
    match_options = [
        f"适合重点关注这类机会的主体包括{audience}。建议把{policy_focus}拆成几个判断题：是否符合申报主体，项目是否在规定周期内，费用是否能归集，成果是否可量化，是否存在重复申报限制。这样做能在申报前先判断成功概率。",
        f"从企业匹配角度看，{audience}不能只看政策标题，而要逐项核对{policy_focus}。如果项目投入、业务成果和政策要求之间缺少对应关系，即使材料很多也未必有效；反过来，平时留痕清楚的企业往往能更快形成申报方案。",
    ...
    company_options = [
        f"深圳金赋的补贴平台可以先帮助{audience}做企业画像和资质评估，再把政策拆成申报条件、材料清单、办理节点和风险提醒。平台沉淀全国四级政府公开扶持政策数据，可按地区、行业、规模、资质、研发投入、知识产权、项目费用和时间窗口进行匹配。",
    ...
    if style == 0:
        ...
        paragraphs = [
            ...
            f"风险一：主体不匹配。{audience}要先核对注册地、行业属性、资质状态和项目周期，不能只因为标题相关就投入申报。{match_options[(variant + 5) % len(match_options)]}",
