...
PLAN_BY_POST_ID = {
    "12821085": (
        "南山模型券最高100万，AI项目先测匹配",
        "南山区规上营利性服务业、工业主体，入驻“模力营”“游托邦”的初创型企业，知名公司高管创业企业，以及正在搭建大模型应用、AI智能体或AI硬件的企业",
        "南山区实际经营、数据申报地在南山区、上季度正增长或新入库无同期数、经遴选云服务商平台、实际消耗token金额、大模型应用、AI智能体、AI硬件、50%补贴比例、单季度100万元或50万元上限、年度预算控制",
        "营业执照、南山区经营证明、数据申报证明、云服务商合同或订单、token消耗账单、付款凭证、AI应用或智能体项目说明、AI硬件研发资料、营收或产值数据、财务制度、信用记录、模力营或游托邦入驻证明、创始人履历和股权证明",
    ),
}
...
FALLBACK_POLICY_BY_POST_ID = {
    "12821085": (
        "南山区促进产业高质量发展专项资金——区工业和信息化局分项资金2026年一季度“模型券”项目操作规程（2026年度）",
        "为推动辖区企业加速人工智能技术研发，全面提升数字化、智能化水平，南山区设立模型券。符合条件的企业通过云服务商平台搭建大模型应用、AI智能体或开发AI硬件等人工智能产品的，按照不超过上季度实际投入金额的50%进行补贴，单家企业每季度最高可申请100万元。本项资助属于专项类项目，坚持公平、公开、公正，实行自愿申报、科学决策和绩效评估，受年度资金预算控制。对满足条件的企业，在经遴选的云服务商平台搭建大模型应用、AI智能体或开发AI硬件等人工智能产品的费用给予补贴。上季度月均营收或产值规模1000万元含以上的，按上季度实际消耗token金额的50%比例给予补贴，单季度补贴上限100万元；上季度月均营收或产值规模1000万元以下的，按上季度实际消耗token金额的50%比例给予补贴，单季度补贴上限50万元。申请主体应在南山区实际开展经营活动，具有独立承担民事责任能力，数据申报地在南山区，可为上季度正增长或新入库无同期数的规上营利性服务业、工业主体，或入驻模力营、游托邦的初创型企业，或知名公司高管创业企业。企业需履行数据申报义务、守法经营、诚实守信、有规范财务管理制度，并积极配合区委、区政府相关工作。被纳入严重失信主体名单或失信惩戒措施清单的，以及申请后项目实施地或数据申报地发生变化不再符合条件的，不予资助。"
    ),
}
...
def build_article_from_url(url: str, index: int) -> dict[str, object]:
    ...
    if style == 0:
        paragraphs = [
            f"先看企业最关心的数字：模型券按上季度实际投入金额最高50%补贴，单家企业单季度最高100万元。对已经在南山做大模型应用、AI智能体或AI硬件的团队来说，token消耗不是纯成本，也可能变成专项资金机会。",
            f"原文里的关键口径很清楚：{policy_focus}。尤其是云服务商平台、上季度实际消耗token金额、1000万元营收或产值分档，会直接影响补贴上限是100万元还是50万元。",
            f"深圳金赋成立于2017年，长期为企业做补贴申报政策匹配和材料管理。补贴平台会先把企业注册地、数据申报地、营收产值、token账单和AI项目说明放在一起看，避免只凭感觉判断。",
            f"企业可以先准备{proof_materials}，不用一开始就写复杂材料，先把AI项目真实发生、费用真实消耗、产品真实开发的证据集中起来。",
            f"如果初筛发现符合方向，再进一步测算50%补贴比例和季度上限；如果暂时不够条件，也可以进入培育清单，后续项目上线、token消耗增加或资料补齐后继续匹配。",
            closing_options[(variant + 8) % len(closing_options)],
        ]
    elif style == 1:
        paragraphs = [
            f"南山企业如果正在用大模型做产品、客服、营销、研发或内部提效，这次模型券就值得认真看。它不是泛泛支持AI，而是把大模型应用、AI智能体、AI硬件和token投入写成了可评估的补贴场景。",
            f"简单说，符合条件的企业通过经遴选的云服务商平台发生token消耗，可按50%比例申请补贴；上季度月均营收或产值1000万元以上，单季度上限100万元，1000万元以下上限50万元。",
            f"这时候不要只问“AI项目能不能报”，而要先问三件事：企业数据申报地是否在南山，主体类型是否符合，token账单和项目说明是否能闭环。",
            f"补贴平台可以把这些问题变成一张匹配表。深圳金赋会结合{proof_materials}，帮企业先看申报基础，再判断是否需要补充云服务订单、消耗明细或AI产品成果说明。",
            f"对正在推进AI落地的企业来说，模型券不只是省一部分算力和模型使用成本，更是把人工智能投入纳入年度补贴规划。越早把项目放进平台，后续匹配同类AI政策、数字经济政策和区级专项资金时就越顺。",
            service_options[(variant + 2) % len(service_options)],
        ]
    elif style == 2:
        paragraphs = [
            f"把这项政策换成财务语言，就是“token消耗有没有机会形成资金回流”。南山区模型券属于专项类项目，符合条件的企业按实际消耗金额测算补贴，材料口径一定要站得住。",
            f"原文特别强调上季度实际消耗token金额。这个点很关键，企业不能只说用了大模型，而要能拿出云服务商平台的订单、账单、付款和使用记录，证明费用确实发生。",
            f"企业可以先把云服务合同、token消耗账单、付款凭证、AI应用或智能体功能说明、营收或产值数据拉出来，按季度做一份AI投入台账。台账越清楚，50%补贴比例越容易测算。",
            f"深圳金赋补贴平台适合做前置测算：企业把基础信息录入后，平台先判断是否落在南山区、是否属于可申报主体、是否满足正增长或新入库条件，再看材料缺口。",
            f"如果企业上季度月均营收或产值达到1000万元以上，就要重点关注单季度100万元上限；如果未达到1000万元，也不要忽略50万元上限带来的补贴空间。",
            closing_options[(variant + 5) % len(closing_options)],
        ]
    elif style == 3:
        paragraphs = [
            f"这条模型券对企业负责人很直接：AI已经投入了，token也在消耗，是否有机会拿回一部分成本？对于南山正在搭建大模型应用、AI智能体或AI硬件的企业，最高100万元的季度补贴值得进入经营会议讨论。",
            f"但补贴不是只看AI概念，还要看主体和投入是否对得上。规上营利性服务业、工业主体要看上季度正增长或新入库无同期数；初创企业还要看是否入驻模力营、游托邦，或是否符合高管创业企业条件。",
            f"企业如果正在开发智能客服、行业智能体、AI硬件样机、内部知识库或模型调用产品，建议同步做政策评估。项目说明、消耗账单和付款路径，会直接影响后续能不能整理成合规材料。",
            f"补贴平台会把政策条件和企业现状对应起来：哪些token费用可纳入，哪些云服务商平台符合要求，哪些主体证明需要补齐，哪些营收或产值数据要提前确认。",
            f"深圳金赋服务企业拿补贴，最看重的是“先测再准备”。符合方向就继续完善材料，不符合也能知道差在哪里，避免把AI项目盲目包装成申报材料。",
            value_options[(variant + 1) % len(value_options)],
        ]
    elif style == 4:
        paragraphs = [
            f"如果企业今年想把AI投入做得更划算，别只盯模型效果，也要看看模型券。南山这项专项资金对大模型应用、AI智能体和AI硬件开发给出支持，符合条件的可按实际token投入50%申请补贴。",
            f"最关键的适用范围包括：在南山区实际经营、数据申报地在南山、符合规上营利性服务业或工业主体条件，或者属于模力营、游托邦初创型企业及符合条件的高管创业企业。",
            f"这里有两个容易忽略的点：一是费用要来自经遴选的云服务商平台；二是补贴看上季度实际消耗token金额，不是看企业对AI项目的口头规划。",
            f"深圳金赋补贴平台会先做“能不能匹配”的判断，再提示材料怎么补。企业上传{proof_materials}后，可以更快看到自身条件与政策要求之间的距离。",
            f"对技术团队来说，这是一次规范AI项目留痕的机会；对财务来说，这是一次把token成本和付款凭证对齐的机会；对管理层来说，这是一次把AI投入转化为补贴机会的机会。",
            closing_options[(variant + 3) % len(closing_options)],
        ]
    elif style == 5:
        paragraphs = [
            f"很多企业会觉得：我们只是正常调用模型、开发智能体，和补贴有什么关系？这项模型券给了一个明确答案——在南山通过云服务商平台搭建AI应用，只要主体、投入和材料符合，就可能进入补贴评估。",
            f"补贴金额不是一刀切，而是和上季度实际消耗token金额、月均营收或产值规模相关。1000万元以上单季度上限100万元，1000万元以下单季度上限50万元，补贴比例均按50%测算。",
            f"建议企业先做一张“AI投入小账本”：云服务商、订单编号、token消耗、付款金额、应用名称、智能体功能、硬件研发节点、上季度营收或产值，都放进去。",
            f"补贴平台可以把这张小账本变成政策匹配结果。深圳金赋会帮助企业判断100万元或50万元上限分别适不适用，也会提醒哪些token费用和证明材料需要单独整理。",
            f"如果暂时没有达到高档上限，也不要急着放弃。企业可以先进入政策培育，后续AI应用上线、消耗提升、营收增长或材料补齐时，再重新测算。",
            service_options[(variant + 5) % len(service_options)],
        ]
    elif style == 6:
        paragraphs = [
            f"这项南山模型券，适合用来做一次AI项目自查。自查不用复杂，先看四个词：主体、平台、消耗、成果。",
            f"主体：企业是否在南山区实际经营，数据申报地是否在南山，是否属于正增长或新入库规上主体，或符合模力营、游托邦、高管创业等条件。",
            f"平台：是否通过经遴选云服务商平台搭建大模型应用、AI智能体或开发AI硬件。消耗：上季度token账单、付款凭证和项目用途是否对应得上。",
            f"成果：AI应用是否能说清楚业务场景、功能效果和投入价值。四项能对上，再看补贴空间：50%比例，单季度最高100万元或50万元。",
            f"深圳金赋补贴平台可以把自查结果保存下来，形成企业自己的AI补贴档案。以后遇到模型券、算力券、数字经济专项资金等同类政策，系统也能更快提示。",
            closing_options[(variant + 6) % len(closing_options)],
        ]
    elif style == 7:
        paragraphs = [
            f"对南山AI企业来说，token消耗不只是技术成本，也可能是一项可管理的政策资产。项目开始调用模型时，如果没有同步留好订单、账单和应用说明，后面想申模型券就会很被动。",
            f"这项专项资金采用自愿申报、科学决策和绩效评估管理，说明企业申报时更看重事实和材料是否清楚。AI项目很新，但证明材料不能含糊。",
            f"原文中的重点包括50%补贴比例、单季度最高100万元、1000万元月均营收或产值分档、云服务商平台、token实际消耗、大模型应用、AI智能体和AI硬件。",
            f"深圳金赋建议企业把模型券当成年度AI专项资金规划的一部分：每月归集token消耗，每季度更新营收或产值，项目上线后同步整理功能截图、使用记录和成果说明。",
            f"补贴平台会根据企业资料动态匹配政策，帮助老板、财务和技术负责人看到同一套判断依据，减少“技术说用了、财务找不到票、管理层不知道能不能申”的情况。",
            value_options[(variant + 2) % len(value_options)],
        ]
    elif style == 8:
        paragraphs = [
            f"先说一句实在的：token费用已经花了，如果符合政策条件，就别让它只停留在云服务账单里。南山企业的大模型投入，有机会通过模型券做补贴评估。",
            f"政策支持对象并不泛泛，而是锁定在南山区实际经营、数据申报地在南山的相关企业，同时覆盖模力营、游托邦初创型企业和符合条件的知名公司高管创业企业。",
            f"企业最好现在就把{proof_materials}集中起来，让补贴平台先看一遍。资料齐不齐、口径对不对、补贴上限能不能测算，都会影响后续推进。",
            f"尤其是token消耗金额和上季度月均营收或产值，别凭印象。一个影响补贴基数，一个影响100万元或50万元上限，都关系到企业能评估出的补贴空间。",
            f"深圳金赋做补贴平台，不是让企业追每一条政策，而是帮助企业找到更可能转化为补贴、贴息或奖励的机会。AI投入这种高频成本，当然值得优先放进清单。",
            closing_options[(variant + 1) % len(closing_options)],
        ]
    elif style == 9:
        paragraphs = [
            f"给南山AI企业一个提醒：如果上季度已经发生模型调用、智能体搭建或AI硬件开发投入，这项模型券可以尽快做匹配。单季度最高100万元的空间，不适合只靠人工记忆来判断。",
            f"企业先看自己在哪一档：月均营收或产值1000万元以上，对应最高100万元；1000万元以下，对应最高50万元；两档都按上季度实际消耗token金额的50%比例测算。",
            f"测算之外，还要看材料。云服务商合同、token账单、付款凭证、南山经营证明、数据申报证明和AI项目说明如果分散在不同部门，就要尽快归集。",
            f"补贴平台可以把这些信息变成企业自己的政策画像：哪些条件已满足，哪些材料待补，预计该优先看哪类专项资金。深圳金赋再结合企业情况给出更清晰的申报判断。",
            f"这类补贴最适合提前准备。等通知出来才翻账单、找订单、补项目说明，往往会耽误节奏；现在把资料整理好，后续有窗口就能更快响应。",
    ...
    if article_chars < 780:
        concise_fillers = [
            f"南山这类模型券专项资金最适合提前规划。企业可以把token消耗、AI应用研发、智能体搭建和硬件开发统一放进年度补贴清单，先看哪些已经具备条件，哪些还需要继续养资料。",
            f"从拿补贴的角度看，token补贴不是单独一张账单，而是企业AI研发能力的侧面证明。项目场景、消耗记录、付款路径和成果说明越清楚，平台判断越准确。",
            f"如果企业正在做AI降本增效，这项政策就不该只在技术部门停留。把模型调用支出和专项资金匹配起来，能帮助企业看到AI成本背后的补贴、奖励和配套机会。",
            f"对快速推进AI产品的企业来说，智能体数量、模型调用量和业务场景往往同步增长。现在把token、订单和项目成果核清楚，后面遇到申报窗口时就能少补很多解释材料。",
            f"企业也可以把这次评估当成一次内部协同：技术核项目场景，财务核账单付款，业务核应用效果，管理层确认申报优先级，补贴平台负责把条件和材料串起来。",
            f"如果企业暂时达不到100万元上限，也不用气馁。补贴平台会记录当前差距，等token消耗增加、营收增长或资料补齐后，再重新匹配同类专项资金机会。",
            f"很多企业真正损失的不是补贴金额，而是没有建立AI项目台账。模型券这种高频投入项目，更需要把订单、账单、付款和应用成果持续更新，避免窗口来了却找不到依据。",
            f"这项政策还提醒企业：补贴申报不是临时动作，而是AI项目管理的一部分。平时把资料留好，季度做专项资金测算时，才更容易看见可争取的空间。",
            f"对老板来说，这类政策的价值很直接：已经发生的token支出，有机会通过政策匹配变成资金回流。先测一轮，不耽误项目，也能提前知道机会大小。",
            f"企业如果已经在南山稳定开展AI项目，更应该把token消耗、云服务平台、营收产值和项目成果做一次完整核验。只要基础条件能对上，后续材料准备就有了明确方向。",
        ]
