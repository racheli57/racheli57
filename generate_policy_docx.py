...
def build_article_from_url(url: str, index: int) -> dict[str, object]:
    ...
    fallback_title = f"{source}政策解读（{post_id_from_url(url)}）"
    fetched_title = ""
    fetched_text = ""
    try:
    ...
    if fetched_text:
        summary = fetched_text[:260]
        opening = f"{title}已发布，政策来源为{source}。从政策原文看，企业应先抓住这些核心信息：{summary}。这类政策不能只停留在转发和收藏层面，而要尽快拆成适用对象、申报窗口、资助方式、材料要求、内部责任人和后续跟进节点，同时明确哪些材料已经具备、哪些材料需要补强。"
    else:
        opening = f"{title}已发布，政策来源为{source}。结合该部门常见政策原文结构，企业应重点核验政策对象、申报条件、支持方式、材料清单、审核流程、资金使用和后续监管要求。对于想拿补贴的企业来说，越早把政策拆成可执行任务，越容易在正式申报时形成完整证据链，也越容易判断这项政策是否值得投入顾问、财务和项目团队资源。"
    paragraphs = [
        opening,
        target_guidance,
