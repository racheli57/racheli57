...
def topic_for_source(source: str, url: str) -> tuple[str, str, str]:
    ...
    if "工业" in source or "中小企业" in source:
        return (
            ...
            "深圳金赋的补贴平台沉淀1100万条全国四级政策数据，可按区域、行业、营收、研发投入、知识产权、人员规模和项目阶段做标签匹配。补贴平台则帮助企业形成年度申报地图，判断先做专精特新、高新技术、技改补贴、数字化转型、软件项目还是产业专项，并将申报材料拆成企业内部可以执行的SOP。",
        )
    if "金融" in source:
        return (
...
def build_article_from_url(url: str, index: int) -> dict[str, object]:
    ...
    if fetched_text:
        ...
        opening = f"{policy_name}已经发布，政策来源为{source}。围绕“{article_title}”这个切入点，企业应先从原文中抓住政策对象、申报条件、支持方式、材料要求和办理节奏：{summary}。对想拿补贴的企业来说，第一步不是转发通知，而是判断这条政策能否对应到自己的项目、费用和证明材料。"
    else:
        opening = f"{policy_name}已经发布，政策来源为{source}。围绕“{article_title}”这个切入点，企业应先核验政策对象、申报条件、支持方式、材料清单、审核流程、资金使用和后续监管要求。对想拿补贴的企业来说，第一步不是转发通知，而是判断这条政策能否对应到自己的项目、费用和证明材料。"

    match_paragraph = (
        f"这篇文章重点写给{audience}。企业阅读政策原文时，建议把{policy_focus}拆成可检查的问题：谁能申报、项目发生在什么时间、哪些费用可以归集、成果如何证明、是否存在重复申报限制。这样做的好处是，企业能在申报前就判断成功概率，而不是等材料提交后才发现项目口径和政策口径不一致。"
    )
    material_paragraph = (
        f"材料准备方面，建议优先整理{proof_materials}。同时检查主体名称、项目名称、合同金额、发票金额、付款路径、成果描述和申报口径是否一致。很多企业明明投入真实，却因为票据分散、项目名称不统一、成果资料缺失，导致后续补正成本很高；如果政策涉及评审、审计或现场核查，还要提前准备项目背景、实施过程、投入明细、绩效成果和风险说明。"
    )
    company_paragraph = (
        f"深圳金赋的补贴平台正是为这类问题服务：先帮助{audience}做企业画像和资质评估，再把政策拆成申报条件、材料清单、办理节点和风险提醒。平台沉淀全国四级政府公开扶持政策数据，能够按地区、行业、企业规模、资质、研发投入、知识产权、项目费用和时间窗口进行匹配；企业还可以通过政策顾问和服务商生态，获得申报规划、材料辅导、流程跟进和合规审查支持。"
    )
    value_paragraph = (
        f"对企业来说，{article_title}不是单点信息，而是一次梳理补贴机会的入口。企业可以借助补贴平台同步排查同区域、同产业、同项目阶段下的其他政策，判断是否存在区级配套、市级专项、省级扶持或国家级资质培育机会。把日常经营、研发、市场、人才、融资和合规投入沉淀为可申报项目，才是持续拿补贴的关键。"
    )
    closing = f"如果企业希望围绕{article_title}判断是否值得申报，并生成更贴合自身情况的补贴推荐清单，{CTA}"
    paragraphs = [opening, match_paragraph, material_paragraph, company_paragraph, value_paragraph, closing]
    return {
        "title": title,
        "source": source,
...
def choose_article_angle(title: str) -> tuple[str, str, str]:
    ...
    if "专精特新" in title or "企业培育" in title or "单项冠军" in title:
        return (
            ...
            "深圳金赋服务企业政策匹配时，会把当下可申报补贴和未来资质培育路径一起看。补贴平台可按区域、行业、营收、研发投入、知识产权、社保人数和资质进度进行标签匹配，补贴平台则帮助企业生成年度申报地图，识别先补哪些短板、先准备哪些材料、哪些时间节点必须跟进。",
        )
    return (
        "企业如何快速判断申报价值？",
